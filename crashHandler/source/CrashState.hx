...
class CrashState extends flixel.FlxState {
    ...
    override function create() {
        ...
        function onClose() {
            ...
            openURL('https://github.com/Stefan2008Git/FNF-SB-Engine/issues');
        }

        Application.current.window.focus();
        Application.current.window.onClose.add(onClose, false, 0);

        var bg:FlxSprite = new FlxSprite().loadGraphic(returnGraphic('crashBG'));
        bg.scrollFactor.set();
        ...
        var logo:FlxSprite = new FlxSprite().loadGraphic(returnGraphic('crashLogo'));
        ...
        for (i=>check in ['Character.hx', 'Song.hx', 'StageData.hx', 'WeekData.hx', 
            'HealthIcon.hx', 'CreditsState.hx', 'DialogueBox.hx', 'FreeplayState.hx',
            'FunkinLua.hx', 'Note.hx'])
        {
        ...
        var text:FlxText = new FlxText(5, 5, 450 * 1.75, (log != null ? log : "You dont have any crash!"), 24).setFormat(null, 20, 0xffff0000, LEFT, OUTLINE, flixel.util.FlxColor.BLACK);
        text.alpha = 0;
        add(text);
    ...
    function getPath(folder:String, key:String):String {
        ...
        return sys.FileSystem.absolutePath(path).contains('crashHandler/export') ? path : 'assets/$folder/crash/$key';
