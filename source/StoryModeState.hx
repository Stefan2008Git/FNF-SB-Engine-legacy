...
class StoryModeState extends MusicBeatState {
	...
	override function create() {
		...
		switch (ClientPrefs.gameStyle) {
			case 'Psych Engine' | 'Better UI' | 'Forever Engine': scoreText.setFormat("VCR OSD Mono", 32);
			default: /*a.k.a "SB Engine"*/ scoreText.setFormat("Bahnschrift", 32);
		}
		
		txtWeekTitle = new FlxText(FlxG.width * 0.7, 10, 0, "", 32);
		switch (ClientPrefs.gameStyle) {
			case 'Psych Engine' | 'Better UI' | 'Forever Engine': txtWeekTitle.setFormat("VCR OSD Mono", 32, FlxColor.WHITE, RIGHT);
		...
		switch (ClientPrefs.gameStyle) {
			case 'Psych Engine' | 'Better UI' | 'Forever Engine': rankText.setFormat("VCR OSD Mono", 32);
