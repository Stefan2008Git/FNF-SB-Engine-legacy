...
class ChartingState extends MusicBeatState {
	...
	var daquantspot = 0;
	var currentlyEventSelected:Int = 0;
	var curUndoIndex = 0;
	var curRedoIndex = 0;
	var _song:SwagSong;
	/*
	...
	**/
	var currentlySelectedNote:Array<Dynamic> = null;
	...
	function addSongUI():Void {
		...
		var directories:Array<String> = [
			Paths.mods('characters/'), Paths.mods(Paths.currentModDirectory + '/characters/'), SUtil.getPath() + Paths.getPreloadPath('characters/')
		];
		...
		var gfVersionDropDown = new FlxUIDropDownMenuCustom(player1DropDown.x, player1DropDown.y + 40, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), 
		    function(character:String) {
		...
		if (skin == null)
			skin = '';
		noteSkinInputText = new FlxUIInputText(player2DropDown.x, player2DropDown.y + 50, 250, skin, 8);
		noteSkinInputText.focusGained = () -> FlxG.stage.window.textInputEnabled = true;
		blockPressWhileTypingOn.push(noteSkinInputText);

		noteSplashesInputText = new FlxUIInputText(noteSkinInputText.x, noteSkinInputText.y + 35, 250, _song.splashSkin, 8);
	...
	function addSectionUI():Void {
		...
		check_gfSection = new FlxUICheckBox(100, 15, null, null, "GF section", 100);
		check_gfSection.name = 'check_gf';
		check_gfSection.checked = _song.notes[curSec].gfSection;
		// _song.needsVoices = check_mustHit.checked;

		check_altAnim = new FlxUICheckBox(check_gfSection.x + 170, check_gfSection.y, null, null, "Alt Animation", 100);
	...
	function addNoteUI():Void {
		...
		strumTimeInputText = new FlxUIInputText(10, 65, 250, "0");
		...
		noteTypeDropDown = new FlxUIDropDownMenuCustom(10, 105, FlxUIDropDownMenuCustom.makeStrIdLabelArray(displayNameList, true),
			function(character:String) {
				...
				if (currentlySelectedNote != null && currentlySelectedNote[1] > -1) {
					currentlySelectedNote[3] = noteTypeIntMap.get(currentType);
					updateGrid();
				}
			});
	...
	function addEventsUI():Void {
		...
		eventDropDown = new FlxUIDropDownMenuCustom(20, 50, FlxUIDropDownMenuCustom.makeStrIdLabelArray(leEvents, true), function(pressed:String) {
			...
			if (currentlySelectedNote != null && eventStuff != null) {
				if (currentlySelectedNote != null && currentlySelectedNote[2] == null) {
					currentlySelectedNote[1][currentlyEventSelected][0] = eventStuff[selectedEvent][0];
				}
				updateGrid();
			}
		});
		...
		tab_group_event.add(text);
		value2InputText = new FlxUIInputText(20, value1InputText.y + 120, 250, "");
		value2InputText.focusGained = () -> FlxG.stage.window.textInputEnabled = true;
		blockPressWhileTypingOn.push(value2InputText);

		// New event buttons
		var removeButton:FlxButton = new FlxButton(eventDropDown.x + eventDropDown.width + 10, eventDropDown.y, '-', function() {
			if (currentlySelectedNote != null && currentlySelectedNote[2] == null) // Is event note
			{
				if (currentlySelectedNote[1].length < 2) {
					_song.events.remove(currentlySelectedNote);
					currentlySelectedNote = null;
				} else {
					currentlySelectedNote[1].remove(currentlySelectedNote[1][currentlyEventSelected]);
				}

				var eventsGroup:Array<Dynamic>;
				--currentlyEventSelected;
				if (currentlyEventSelected < 0)
					currentlyEventSelected = 0;
				else if (currentlySelectedNote != null && currentlyEventSelected >= (eventsGroup = currentlySelectedNote[1]).length)
					currentlyEventSelected = eventsGroup.length - 1;
		...
		var addButton:FlxButton = new FlxButton(removeButton.x + removeButton.width + 10, removeButton.y, '+', function() {
			if (currentlySelectedNote != null && currentlySelectedNote[2] == null) // Is event note
			{
				var eventsGroup:Array<Dynamic> = currentlySelectedNote[1];
				eventsGroup.push(['', '', '']);
	...
	function changeEventSelected(change:Int = 0) {
		if (currentlySelectedNote != null && currentlySelectedNote[2] == null) // Is event note
		{
			currentlyEventSelected += change;
			if (currentlyEventSelected < 0)
				currentlyEventSelected = Std.int(currentlySelectedNote[1].length) - 1;
			else if (currentlyEventSelected >= currentlySelectedNote[1].length)
				currentlyEventSelected = 0;
			selectedEventText.text = 'Selected Event: ' + (currentlyEventSelected + 1) + ' / ' + currentlySelectedNote[1].length;
		} else {
			currentlyEventSelected = 0;
	...
	function addChartingUI() {
		...
		#if !html5
		sliderRate = new FlxUISlider(this, 'playbackSpeed', 120, 120, 0.5, 3, 190, null, 5, FlxColor.WHITE, FlxColor.BLACK);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if (id == FlxUICheckBox.CLICK_EVENT) {
			...
			if (wname == 'section_beats') {
				...
				if (currentlySelectedNote != null && currentlySelectedNote[2] != null) {
					currentlySelectedNote[2] = nums.value;
					updateGrid();
				}
			...
			} else if (currentlySelectedNote != null) {
				if (sender == value1InputText) {
					if (currentlySelectedNote[1][currentlyEventSelected] != null) {
						currentlySelectedNote[1][currentlyEventSelected][1] = value1InputText.text;
						updateGrid();
					}
				} else if (sender == value2InputText) {
					if (currentlySelectedNote[1][currentlyEventSelected] != null) {
						currentlySelectedNote[1][currentlyEventSelected][2] = value2InputText.text;
						updateGrid();
					}
...
=======
						    addNote(currentlySelectedNote[0] + (_song.notes[curSec].changeBPM ? 15000/_song.notes[curSec].bpm : 15000/_song.bpm)/stepperStackOffset.value, currentlySelectedNote[1], currentType);
>>>>>>> 64cc2b95e45df545249931ba5d557fbfc37aab24
					    }
					}
			}
		}
		...
		if (!blockInput) {
			...
			if ((FlxG.keys.justPressed.Z #if android || virtualPad.buttonZ.justPressed #end)
				&& curZoom > 0
			...
			if (FlxG.mouse.wheel != 0) {
				...
				if (!mouseQuant)
					...
					if (FlxG.mouse.wheel > 0) {
						var freak:Float = CoolUtil.quantize(beat, snap) - increase;
						FlxG.sound.music.time = Conductor.beatToSeconds(freak);
					} else {
						var freak:Float = CoolUtil.quantize(beat, snap) + increase;
						FlxG.sound.music.time = Conductor.beatToSeconds(freak);
					}
			...
			if (FlxG.keys.pressed.W || FlxG.keys.pressed.S #if android || virtualPad.buttonUp.pressed || virtualPad.buttonDown.pressed #end) {
				...
				if (FlxG.keys.pressed.CONTROL)
					holdingShift = 0.25;
				else if (FlxG.keys.pressed.SHIFT #if android || virtualPad.buttonY.pressed #end)
					holdingShift = 4;
			...
			if (!vortex) {
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN) {
					...
					if (FlxG.keys.pressed.UP) {
						var freak:Float = CoolUtil.quantize(beat, snap) - increase; // (Math.floor((beat+snap) / snap) * snap);
						FlxG.sound.music.time = Conductor.beatToSeconds(freak);
					} else {
						var freak:Float = CoolUtil.quantize(beat, snap) + increase; // (Math.floor((beat+snap) / snap) * snap);
						FlxG.sound.music.time = Conductor.beatToSeconds(freak);
			...
			if (vortex && !blockInput) {
				...
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN #if android || virtualPad.buttonUp.justPressed || virtualPad.buttonDown.justPressed #end) {
					...
					if (FlxG.keys.pressed.UP #if android || virtualPad.buttonUp.pressed #end) {
						var freak:Float = CoolUtil.quantize(beat, snap) - increase;
						feces = Conductor.beatToSeconds(freak);
					} else {
						var freak:Float = CoolUtil.quantize(beat, snap) + increase; // (Math.floor((beat+snap) / snap) * snap);
						feces = Conductor.beatToSeconds(freak);
					}
					...
					if (currentlySelectedNote != null) {
						dastrum = currentlySelectedNote[0];
					}
					...
					if (currentlySelectedNote != null) {
						...
						if (controlArray.contains(true)) {
							for (i in 0...controlArray.length) {
								if (controlArray[i])
									if (currentlySelectedNote[1] == i)
										currentlySelectedNote[2] += datime - currentlySelectedNote[2] - Conductor.stepCrochet;
							}
							updateGrid();
							updateNoteUI();
						}
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			...
			if (currentlySelectedNote != null) {
				...
				if (currentlySelectedNote[0] == note.strumTime
					&& ((currentlySelectedNote[2] == null && noteDataToCheck < 0)
						|| (currentlySelectedNote[2] != null && currentlySelectedNote[1] == noteDataToCheck))) {
					colorSine += elapsed;
	...
	function changeNoteSustain(value:Float):Void {
		if (currentlySelectedNote != null) {
			if (currentlySelectedNote[2] != null) {
				currentlySelectedNote[2] += value;
				currentlySelectedNote[2] = Math.max(currentlySelectedNote[2], 0);
			}
		}
	...
	function updateNoteUI():Void {
		if (currentlySelectedNote != null) {
			if (currentlySelectedNote[2] != null) {
				stepperSusLength.value = currentlySelectedNote[2];
				if (currentlySelectedNote[3] != null) {
					currentType = noteTypeMap.get(currentlySelectedNote[3]);
					if (currentType <= 0) {
						noteTypeDropDown.selectedLabel = '';
					} else {
						noteTypeDropDown.selectedLabel = currentType + '. ' + currentlySelectedNote[3];
					}
				}
			} else {
				eventDropDown.selectedLabel = currentlySelectedNote[1][currentlyEventSelected][0];
				var selected:Int = Std.parseInt(eventDropDown.selectedId);
				if (selected > 0 && selected < eventStuff.length) {
					descText.text = eventStuff[selected][1];
				}
				value1InputText.text = currentlySelectedNote[1][currentlyEventSelected][1];
				value2InputText.text = currentlySelectedNote[1][currentlyEventSelected][2];
			}
	...
	function selectNote(note:Note):Void {
		...
		if (noteDataToCheck > -1) {
			if (note.mustPress != _song.notes[curSec].mustHitSection)
				noteDataToCheck += 4;
			for (i in _song.notes[curSec].sectionNotes) {
				if (i != currentlySelectedNote && i.length > 2 && i[0] == note.strumTime && i[1] == noteDataToCheck) {
					currentlySelectedNote = i;
					break;
				}
			}
		} else {
			for (i in _song.events) {
				if (i != currentlySelectedNote && i[0] == note.strumTime) {
					currentlySelectedNote = i;
					currentlyEventSelected = Std.int(currentlySelectedNote[1].length) - 1;
					break;
				}
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void {
		...
		if (noteData > -1) {
			...
			currentlySelectedNote = _song.notes[curSec].sectionNotes[_song.notes[curSec].sectionNotes.length - 1];
		} else {
			var event = eventStuff[Std.parseInt(eventDropDown.selectedId)][0];
			var text1 = value1InputText.text;
			var text2 = value2InputText.text;
			_song.events.push([noteStrum, [[event, text1, text2]]]);
			currentlySelectedNote = _song.events[_song.events.length - 1];
			currentlyEventSelected = 0;
		}
		changeEventSelected();
	...
	var missingFileTextTimer:FlxTimer;
	function loadJson(song:String):Void
	{
		...
		catch(e)
		{
			...
			else missingFileTextTimer.cancel();

			missingFileText.text = 'Error on loading song chart:\n$fileNotFound';
			missingFileText.screenCenter(Y);

			missingFileTextTimer = new FlxTimer().start(5, function(tmr:FlxTimer) {
				remove(missingFileText);
				missingFileText.destroy();
			});
